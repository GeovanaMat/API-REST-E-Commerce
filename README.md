...
erDiagram
    CUSTOMER {
        Long id PK "ID do Cliente"
        string name "Nome"
        string email "Email"
    }
    PROFILE {
        Long id PK "ID do Perfil"
        Long customerId FK "ID do Cliente"
        string address "Endereço"
        string phone "Telefone"
    }
    CATEGORY {
        Long id PK "ID da Categoria"
        string name "Nome"
    }
    PRODUCT {
        Long id PK "ID do Produto"
        string name "Nome"
    ...
    ORDER {
        Long id PK "ID do Pedido"
        Long customerId FK "ID do Cliente"
        date orderDate "Data do Pedido"
        decimal totalAmount "Total de quantity*PRODUCT.price de todos os order_item"
    }
    ORDER_ITEM {
        Long id PK "ID do Item do Pedido"
        Long orderId FK "ID do Pedido"
        Long productId FK "ID do Produto"
